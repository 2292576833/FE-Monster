...
if ($script:fixtureWingetCalls -ne 1) {
  throw 'A damaged registered WebView2 runtime did not enter the repair chain through winget.'
}
if ($script:fixtureBootstrapperCalls -ne 1) {
  throw 'A damaged registered WebView2 runtime did not reach the official Microsoft bootstrapper fallback.'
}
