...
const scratch = mkdtempSync(path.join(os.tmpdir(), 'fe-achievement-community-sync-'));
...
try {
  ...
  run(javac, [
    ...
    path.join(root, 'src', 'test', 'java', 'com', 'femonster', 'core', 'AchievementStateServiceSyncProbe.java'),
    path.join(root, 'src', 'test', 'java', 'com', 'femonster', 'core', 'AchievementCommunitySyncIntegrationProbe.java')
  ]);
  const localOutput = run(java, [
    '-cp', `${testClasses}${path.delimiter}${mainClasses}`,
    'com.femonster.core.AchievementStateServiceSyncProbe'
  ]);
  assert.match(localOutput, /AchievementStateServiceSyncProbe passed/);
  const output = run(java, [
    '-cp', `${testClasses}${path.delimiter}${mainClasses}`,
    'com.femonster.core.AchievementCommunitySyncIntegrationProbe',
  ...
  assert.match(output, /AchievementCommunitySyncIntegrationProbe passed/);

  const database = JSON.parse(readFileSync(path.join(serverData, 'community-db.json'), 'utf8'));
  assert.equal(Object.keys(database.achievementStates || {}).length, 2,
    'the signed Java path did not persist one achievement state per account');
