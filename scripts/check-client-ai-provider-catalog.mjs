...
const scratch = path.join(root, 'tmp', 'client-ai-provider-catalog-probe');
...
try {
  run(javac, [
    ...
    path.join(root, 'src/test/java/com/femonster/ai/AiProviderCatalogProbe.java'),
  ]);
  const output = run(java, ['-cp', classes, 'com.femonster.ai.AiProviderCatalogProbe']);
  assert.match(output, /AiProviderCatalogProbe passed/);
  console.log(output);
} finally {
  rmSync(scratch, { recursive: true, force: true });
}
