...
const output = path.join(root, '.tmp', 'provider-protocol-contract-classes');
const appContext = readFileSync(
  path.join(root, 'src/main/java/com/femonster/core/AppContext.java'),
  'utf8'
);

if (!appContext.includes('new ProviderProtocolClient(')) {
  throw new Error('Configured QQ and Kugou plugins do not use the explicit provider protocol');
}
for (const provider of ['qq', 'kugou']) {
  if (appContext.includes(`new GenericMusicClient("${provider}"`)) {
    throw new Error(`${provider} still uses the endpoint-guessing client`);
  }
}
...
try {
  run(executable('javac'), [
    ...
    path.join(root, 'scripts/java/ProviderProtocolContractProbe.java')
  ]);
  console.log(run(executable('java'), [
    '--add-modules', 'jdk.httpserver',
    '-cp', output,
    'ProviderProtocolContractProbe'
  ]));
} finally {
  rmSync(output, { recursive: true, force: true });
}
