...
const output = path.join(root, '.tmp', 'official-browser-account-switch-classes');
...
try {
  run(executable('javac'), [
    ...
    path.join(root, 'src/main/java/com/femonster/core/OfficialBrowserLoginService.java'),
    path.join(root, 'scripts/java/OfficialBrowserAccountSwitchProbe.java')
  ]);
