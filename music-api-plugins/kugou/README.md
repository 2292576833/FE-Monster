...
3. 使用酷狗音乐 App 扫码并确认。插件会用同一用户 ID 和同一标准版平台自动续期一次性授权，并在读取头像、VIP 与歌单成功后才确认登录；会话只保存在本地插件数据目录，不会传回网页界面。

## 数据目录

宿主通过 `--data-dir=${data}/kugou-music-api` 分配独立数据目录。插件在其中原子写入 `session.json`，保存经酷狗 App 确认的账户会话、`dfid` 与匿名设备标识。请把该文件当作登录凭据保护，不要公开或提交到版本库。
...
- 由 FE Monster 官方浏览器展示酷狗官方二维码，插件私下完成授权轮询与会话持久化
...
错误 JSON 使用 `type: "api"` 或 `type: "network"`，并保留 `errorType` 兼容旧宿主。`/health` 返回 provider、版本、经验证的登录状态、`authMode: "provider-qr"` 与能力列表，不会泄露 Cookie、二维码授权结果中的令牌或原始异常对象。
