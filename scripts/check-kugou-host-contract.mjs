...
const output = path.join(root, '.tmp', 'kugou-host-contract-classes');
const genericSource = readFileSync(path.join(root, 'src/main/java/com/femonster/music/GenericMusicClient.java'), 'utf8');
if (genericSource.includes('kugouSongUrlFallback') || genericSource.includes('kugouSearchFallback')) {
  throw new Error('Kugou requests still bypass the imported plugin through a legacy direct endpoint');
}
...
try {
  run(executable('javac'), [
    ...
    path.join(root, 'scripts/java/KugouHostContractProbe.java')
  ]);
  console.log(run(executable('java'), [
    '--add-modules', 'jdk.httpserver',
    '-cp', output,
    'KugouHostContractProbe'
  ]));
} finally {
  rmSync(output, { recursive: true, force: true });
}
