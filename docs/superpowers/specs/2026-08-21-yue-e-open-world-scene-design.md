...
1. 本文的书面尺寸、职责、交互、安全和验收规则；
2. Gate 1 最终获批的原创人物三视图、背面图、材质色板和 UE 实时转台；
3. 用户提供的 `CharacterReference-01` 拼图，只约束卡通效果和服装语言；
4. 动作、镜头和地图概念图，只表达姿态或构图意图，不能冒充真实三维验收。

旧 `docs/superpowers/assets/yue-e/e-traveler-approved.png` 以及旧规格中的琥珀眼缝、独立玻璃虫翼、白衣蓝裤角色属于已废止 Three.js 方案，不再是生产外观来源。

`CharacterReference-01` 是用户提供的 `share_10a4121c1a71fb9d6d86629cdb6df70f.png`，SHA-256 为 `49D58F180BEF937EDD3DDD682CEAACCCE903F9E83C2B02FC6E94CEE39431183A`。Gate 0 必须把原始字节归档到访问受控、只读、非发布资产的项目制品库，并建立包含稳定 URI、大小、MIME 和 SHA-256 的参考 manifest；本地下载路径本身不算归档。只有用户明确授权时才可把副本纳入代码仓库，且副本哈希必须相同。若因授权、源文件丢失或哈希不符而无法保存可读取的原始字节，Gate 0 必须保持 `in_progress` 并请求用户重新提供或授权，不能只凭文件名或哈希进入 Gate 1。
...
用户提供的人物拼图作为 `CharacterReference-01`，约束：
...
- 胸口或额头克制的暖色发光符号；
- 大色块斗篷、上衣和宽松下装；
- 柔和卡通材质、干净轮廓、暖边光和轻微辉光；
- 不出现皮肤毛孔、真实发丝、真人眼鼻口或写实人体比例。

首发只锁定一套默认母版：深色无五官面罩、珍珠白层叠短发、象牙色上衣、夕阳橙斗篷、深色腿部、奶白软靴和暖琥珀胸口光核。滑翔时由同一件橙色斗篷展开为六段宽阔发光翼面；不再叠加独立虫翼或玻璃翅膀。

“严格按照参考图效果”表示严格执行比例、轮廓、材质语言和氛围，不表示复制原游戏资产。所有网格、拓扑、骨骼、UV、材质图、动画和服装模块都必须从零原创制作。
...
- 约 4.5–5 头身，头部较大、肩窄、躯干短、四肢清楚；
- 深靛蓝近黑无五官面罩，只允许一个可关闭的极小额饰发光点；
- 珍珠白层叠短发和柔软侧发束，不使用真实发丝；
- 象牙色短上衣、夕阳橙斗篷、深色腿部和奶白软靴；
- 胸口暖琥珀光核；
- 略大的圆润手套形手部和鞋靴；
- 橙色斗篷内置六段发光翼面，地面时收拢，滑翔时展开为宽阔卡通翼形。

发型、斗篷、上衣、下装、鞋靴、额饰和胸口符号预留模块化插槽，但 Gate 1 只生产和验收这一套默认造型。所有模块共用批准骨架、穿插规范和 LOD 契约。

### 7.2 模型预算

- LOD0 目标 45k–80k 三角形；LOD1 约 50%，LOD2 约 20%；
- 变形骨骼建议不超过 120，单顶点权重不超过 4；
- 角色、头发、斗篷、鞋靴和光核必须有真实厚度、自遮挡、阴影和材质响应；
- 不使用面向镜头的角色身体、头发或斗篷贴片；
...
- 4K 近景图展示面罩、头发、斗篷、胸口光核和鞋靴；
- 与 `CharacterReference-01` 并排说明比例和视觉语言；
...
- 不允许关节弹跳、斗篷穿身、IK 突跳、固定帧率计时或动画硬切；
...
- 把 `CharacterReference-01` 原始字节归档到受控只读制品库并验证 SHA-256；同时归档 `WorldMapReference-01`、`SceneUIManifest-v1` 和 `PresetManifest-v1` 的稳定 ID、可读取位置与哈希，作为后续 Gate 可复现验收基线；无法取得参考原始字节时 Gate 0 阻塞并请求用户重新提供或授权。

### Gate 1：人物立体造型

- 完成唯一默认角色的原创 Blender 模型、骨骼、蒙皮、UV、LOD、碰撞和 sockets；
- FBX 无动画、无最终材质/贴图，UE 三个对应导入选项关闭；
- UE 完成角色材质、光核、六段斗篷翼面和灯光转台；
