...
const scratch = path.join(root, 'tmp', 'pet-personalization-snapshot-probe');
...
try {
  run(javac, [
    ...
    path.join(root, 'src/test/java/com/femonster/core/PetPersonalizationSnapshotProbe.java'),
  ]);
  const output = run(java, ['-cp', classes, 'com.femonster.core.PetPersonalizationSnapshotProbe']);
  assert.match(output, /PetPersonalizationSnapshotProbe passed/);
  console.log(output);
} finally {
  rmSync(scratch, { recursive: true, force: true });
}
