...
const output = path.join(root, '.tmp', 'provider-identity-account-runtime-classes');
...
try {
  run(executable('javac'), [
    ...
    path.join(root, 'scripts/java/ProviderIdentityAccountProbe.java')
  ]);
  console.log(run(executable('java'), [
    '--add-modules', 'jdk.httpserver',
    '-cp', output,
    'ProviderIdentityAccountProbe'
  ]));
} finally {
  rmSync(output, { recursive: true, force: true });
}
